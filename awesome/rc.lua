...
run_once("utox")
run_once("kshutdown --shutdown 22:00 --init")
-- }}}
...
editor     = os.getenv("EDITOR") or "emacs" or "nano"
...
mailwidget = lain.widgets.abase({
   timeout = 30,
    cmd = "python /home/kzer-za/mailcheck",
    settings = function()
       widget:set_text(" " .. output .. " ")
       -- if n % 2 == 0 then
       -- 	  outputs["val1"] = output
       -- else
       -- 	  outputs["val2"] = outputs["val1"]
       -- end
       -- n = n + 1
       
       -- if tonumber(outputs["val2"]) > tonumber(outputs["val2"]) then
       -- 	  naughty.notify({ text="New email received!", icon="" })
       -- end
    end
})
-- MPD
...
for s = 1, screen.count() do
    ...
    right_layout_add(pkgicon, pkgwidget)    
    ...
    right_layout_add(myweather.icon, myweather)
            myweather.update()
...
globalkeys = awful.util.table.join(
   ...
   awful.key({ altkey }, "r", function() os.execute("/home/kzer-za/.screenlayout/refresh.sh && /home/kzer-za/.screenlayout/default.sh") end),
   awful.key({ altkey, "Shift" }, "1", function()
		os.execute("pluck add blackout")
		naughty.notify({ text="Blackout begin", icon="" }) end),
   awful.key({ altkey, }, "1", function()
		os.execute("pluck rm blackout")
		naughty.notify({ text="Blackout end", icon="" }) end),
   awful.key({ altkey }, "/", function() os.execute("scrot") end),
